...
def compute_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not agg_spec:
        empty = (
            df.groupby("Operator_ID").size().reset_index(name="Event_Count")
        )
        empty["Suspicion_Score"] = 0
        empty["Risk_Level"] = "Low"
        return empty[["Operator_ID", "Suspicion_Score", "Risk_Level"]]
