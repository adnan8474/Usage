...
def apply_flags(
    ...
    """Apply all flagging rules with parameterised thresholds."""
    df = rapid_succession(df, rapid_th)
    df = location_conflict(df)
    df = hourly_density(df)
    df = device_hopping(df, hop_threshold, window_minutes)
...
def ensure_unique_event_id(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure an Event_ID column exists and is unique for each record."""
    df = df.copy()
    if 'Event_ID' not in df.columns:
        df['Event_ID'] = range(1, len(df) + 1)
    else:
        df['Event_ID'] = pd.factorize(df['Event_ID'])[0] + 1
    return df
...
def compute_all_flags(
    ...
    """Run all flagging rules and mark combined Flagged boolean."""
    df = apply_flags(df, rapid_th, hop_threshold, window_minutes)
    df['Flagged'] = df[FLAG_COLUMNS].any(axis=1)
    return df
